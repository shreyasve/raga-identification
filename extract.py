    ...
    try:
        ...
        if dur <= 0:
            ...
            for i in range(num_segments):
                ...
                rolloff = np.mean(librosa.feature.spectral_rolloff(y=x, sr=sr))  # Fixed
                zcr = np.mean(librosa.feature.zero_crossing_rate(y=x))  # Fixed

                mfccs = librosa.feature.mfcc(y=x, sr=sr, n_mfcc=20)
                mfcc_means = np.mean(mfccs, axis=1)

                data = [
                    os.path.basename(audio_file) + f"-{i+1}", rmse, chroma_stft, spec_cent, spec_bw, rolloff, zcr,
